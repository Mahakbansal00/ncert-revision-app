...
export default function SourceSelector({ selected, onSelect }) {
  ...
  const samples = [
    {
      name: "NCERT Class XI Physics (sample)",
      path: "/pdfs/sample_ncert_class11_physics.pdf",
    },
  ];
  ...
  return (
    <div className="bg-white p-4 rounded shadow">
      ...
      <div className="space-y-2">
        <button
          onClick={() => onSelect("all-uploaded")}
          className={`w-full text-left p-2 rounded ${
            selected === "all-uploaded" ? "bg-indigo-50" : ""
          }`}
        >
          All uploaded PDFs
        </button>
        ...
        {uploadedPdfs.map((s) => (
          <div
            key={s.path}
            className="flex items-center justify-between w-full rounded hover:bg-gray-50 p-2"
          >
            <button
              onClick={() => onSelect(s.path)}
            ...
            <button
              onClick={() => {
                const updated = uploadedPdfs.filter(
                  (pdf) => pdf.path !== s.path
                );
                setUploadedPdfs(updated);
                localStorage.setItem("uploadedPdfs", JSON.stringify(updated));
      ...
      <div className="mt-3">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {uploading ? "Uploading..." : "Upload your own PDF coursebook."}
        </p>
      </div>
    </div>
  );
